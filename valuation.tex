\begin{frame}{Valuing Investment Projects with Leverage}

\begin{block}{Core Question}
How should we value a project when financing affects taxes, cash flows,
and risk?
\end{block}

\textbf{Why leverage complicates valuation:}
\begin{itemize}
    \item Debt creates interest tax shields.
    \item Equity becomes riskier when leverage increases.
    \item Financing side effects may change project value.
\end{itemize}

\textbf{Three approaches:}
\begin{itemize}
    \item \textbf{APV}: value the project first, then add financing effects.
    \item \textbf{FTE}: value cash flows paid to equityholders.
    \item \textbf{WACC}: discount project cash flows using a blended cost of capital.
\end{itemize}

\vfill

\begin{alertblock}{Key Idea}
Levered valuation is mostly about matching the right cash flow with the
right discount rate.
\end{alertblock}
...
\begin{table}
\centering
\begin{tabular}{p{2.4cm}p{5.0cm}p{3.0cm}}
\toprule
\textbf{Method} & \textbf{Cash Flow Used} & \textbf{Discount Rate} \\
\midrule
\textbf{APV}  & Unlevered CF, plus financing effects separately & $R_0$ \\
\textbf{WACC} & Unlevered CF & WACC \\
\textbf{FTE}  & Levered CF to equityholders & $R_S$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{block}{Tax Shield Treatment}
APV values tax shields separately; WACC builds them into the discount rate;
FTE embeds them in equity cash flows.
\end{block}

\vfill

\begin{alertblock}{Bottom Line}
All three methods should give the same answer if applied consistently.
\end{alertblock}
