...
\begin{frame}{Summary: Capital Structure and Leverage}

\begin{block}{Capital Structure Question}
Can a firm increase total value by changing its mix of debt and equity?
\end{block}

\vspace{0.3cm}

\begin{itemize}
    \item Capital structure changes how the firm's cash flows are divided.
    \item It creates value only if it increases the total size of the ``pie''.
    \item Financial leverage amplifies equity returns:
    \begin{itemize}
        \item Higher upside when EBIT is high.
        \item Greater downside when EBIT is low.
    \end{itemize}
\end{itemize}
...
Leverage changes the risk and payoff of equity, but this does not
automatically mean that firm value increases.
\end{alertblock}

\end{frame}

\begin{frame}{Summary: Modigliani--Miller Propositions}

\begin{columns}[T,onlytextwidth]

\column{0.48\textwidth}
\begin{block}{No Taxes}
\[
V_L = V_U
\]
...
\begin{itemize}
    \item Capital structure is irrelevant.
    \item Equity risk rises with leverage.
    \item WACC remains unchanged.
\end{itemize}
\end{block}

\column{0.48\textwidth}
\begin{block}{Corporate Taxes}
\[
V_L = V_U + T_C B
\]

\begin{itemize}
    \item Debt creates an interest tax shield.
    \item Firm value increases with permanent debt.
    \item WACC declines with leverage.
\end{itemize}
\end{block}
...
If debt is valuable because of tax shields, why are firms not financed
entirely with debt? Financial distress costs provide the next answer.
\end{alertblock}
